...
//    printf("%d %d %d %d", SEC_IN_MIN, MIN_IN_HOUR, SEC_IN_HOUR, SEC_IN_DAY);
    int h, m, s, original_time = 0;
    long long delta_time, delta_time_h = -1, delta_time_m = -1, delta_time_s = -1;
    long long alarm_time_h = 0, alarm_time_m = 0, alarm_time_s = 0, extra_days = 0, alarm_time = 0;
    //Input original_time
    scanf("%d:%d:%d", &h, &m, &s);
    original_time = h * SEC_IN_HOUR + m * SEC_IN_MIN + s;
    //#Delta time input
    scanf("%lld:%lld:%lld", &delta_time_h, &delta_time_m, &delta_time_s);
